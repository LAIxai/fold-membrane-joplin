...
function repairMupSpan(body: string): string {
  ...
  // ⑥ バックスラッシュ3連以上のみ削除（LaTeX数式はスロット保護済み）
  //    v8.73 [2026.04.18(土)am11:05] 閾値を /\\/g(全削除) → /\\{3,}/g(3個以上)へ変更。
  //    正常なMarkdownエスケープ \\ (=リテラル\1個) を誤って消していた重大バグ修正。
  //    2個以下は正常、3個以上は暴走増殖残骸として削除する。
