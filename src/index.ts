...
<div id="toc-list"></div>
    ...
    await joplin.contentScripts.onMessage('markMupRenderer', async (msg: any) => {
      ...
      if (msg.type === 'mupSetBadgeState') {
        ...
        // v0.9.141: 全バッジを検出して重複クリーンアップ。先頭バッジのcount/expを継承し、
        //          残りのバッジは全削除して行の末尾に新バッジを1個だけ配置。
        const badgeReG = /\s*(?:\[([⊕⊖⊘])(f?)(∞|♾️|\d+)(?:\+(\d+))?\]|\[(∞|♾️)\]|\[(\d+)\])/g;
        const allMatches: RegExpExecArray[] = [];
        let mmx: RegExpExecArray | null;
        while ((mmx = badgeReG.exec(line)) !== null) {
          allMatches.push(mmx);
        }
        if (allMatches.length === 0) return; // バッジ無しは対象外（CN=8153が自動付加するはず）
        // 先頭マッチからcount/expを継承
        const first = allMatches[0];
        let count: string, exp: string;
        if (first[1]) { count = first[3]; exp = first[4] || '0'; }
        else if (first[5]) { count = first[5]; exp = '0'; }
        else { count = first[6]; exp = '0'; }
        const newBadge = (count === '∞' || count === '♾️')
          ? `[${state}${count}]`
          : `[${state}${count}+${exp}]`;
        // 全バッジを行から除去（逆順）→末尾に新バッジを1個追加
        let stripped = line;
        for (let k = allMatches.length - 1; k >= 0; k--) {
          const am = allMatches[k];
          stripped = stripped.slice(0, am.index) + stripped.slice(am.index + am[0].length);
        }
        const newLine = stripped.replace(/\s+$/, '') + ' ' + newBadge;
      ...
      // \▼[CN=5128_onMessage.SET_PFX] // プレフィックス切替 CN⇄H1⇄H2⇄H3
      // mupFold.js v7.11 のプレフィックスインラインボタン行から呼ばれる。
      // 開き膜 [OLDPFX=cn] を [NEWPFX=cn] に置換し、対応する閉じ膜も同時に書換える。
      // 同名入れ子に備えdepth追跡で正しいペアを特定する。
      if (msg.type === 'mupSetPfx') {
        const cn     = String(msg.cn || '');
        const oldPfx = String(msg.pfx || 'CN');
        const newPfx = String(msg.newPfx || '');
        const occurrenceIndex = typeof msg.occurrenceIndex === 'number' ? msg.occurrenceIndex : 0;
        if (!cn || !newPfx || oldPfx === newPfx) return;
        if (!/^(?:CN|H[1-3])$/.test(oldPfx) || !/^(?:CN|H[1-3])$/.test(newPfx)) return;
        const note = await joplin.workspace.selectedNote();
        if (!note?.body) return;
        const escapedCn = cn.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const cnPat = /^\d{4}/.test(cn) ? escapedCn : '(?:\\d{4}_)?' + escapedCn;
        const openRe  = new RegExp('^([ \\t]*\\$?(?:[▼▶]m|M[▼▶]|[▼▶]_M)\\[)' + oldPfx + '=(' + cnPat + ')(\\]\\$?.*)$');
        const closeRe = new RegExp('^([ \\t]*\\$?(?:[▲◀]m|M[▲◀]|[▲◀]_M)\\[)' + oldPfx + '=(' + cnPat + ')(\\]\\$?.*)$');
        const lines = note.body.split('\n');
        // occurrenceIndex 番目の開き膜行を探す
        ...
        if (closeIdx < 0) return;
        // 両方の行のpfxを置換（cn名は保持）
        lines[openIdx]  = lines[openIdx].replace(openRe,  '$1' + newPfx + '=$2$3');
        lines[closeIdx] = lines[closeIdx].replace(closeRe, '$1' + newPfx + '=$2$3');
      ...
      // \▼[CN=2947_onMessage.SET_MTYPE] // 膜形式切替 m ⇄ M (不可侵膜)
      // mupFold.js v7.15 の m/M ボタンから呼ばれる。
      // 開き膜: ▼m[...] ⇄ M▼[...]  /  閉じ膜: ▲m[...] ⇄ M▲[...]
      // _M(legacy)形式は m に正規化してから M へ遷移（元に戻すときは m 形式になる＝legacy解消）。
      if (msg.type === 'mupSetMType') {
        const cn       = String(msg.cn || '');
        const pfx      = String(msg.pfx || 'CN');
        const newMType = String(msg.newMType || '');
        const occurrenceIndex = typeof msg.occurrenceIndex === 'number' ? msg.occurrenceIndex : 0;
        if (!cn) return;
        if (!/^(?:CN|H[1-3])$/.test(pfx)) return;
        if (newMType !== 'm' && newMType !== 'M') return;
        const note = await joplin.workspace.selectedNote();
        if (!note?.body) return;
        const escapedCn = cn.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const cnPat = /^\d{4}/.test(cn) ? escapedCn : '(?:\\d{4}_)?' + escapedCn;
        // 全形式を受け入れる open/close 正規表現（キャプチャ: 1=leading, 2=arrow, 3=[pfx=cn], 4=trailing）
        const openRe  = new RegExp('^([ \\t]*\\$?)(?:([▼▶])m|[Mm🄼]([▼▶])|([▼▶])_[Mm🄼])(\\[' + pfx + '=' + cnPat + '\\]\\$?)(.*)$');
        const closeRe = new RegExp('^([ \\t]*\\$?)(?:([▲◀])m|[Mm🄼]([▲◀])|([▲◀])_[Mm🄼])(\\[' + pfx + '=' + cnPat + '\\]\\$?)(.*)$');
        const lines = note.body.split('\n');
        let openIdx = -1, seen = 0;
        for (let i = 0; i < lines.length; i++) {
          if (openRe.test(lines[i])) {
        ...
        if (closeIdx < 0) return;
        // 置換ヘルパー: 矢印を抽出して新形式に組み立て直す
        const rewrite = (line: string, re: RegExp) => {
          return line.replace(re, (_m: string, lead: string, a1: string, a2: string, a3: string, body: string, trail: string) => {
            const arrow = a1 || a2 || a3 || '';
            const prefix = (newMType === 'M') ? ('M' + arrow) : (arrow + 'm');
            return lead + prefix + body + trail;
          });
        };
        lines[openIdx]  = rewrite(lines[openIdx],  openRe);
        lines[closeIdx] = rewrite(lines[closeIdx], closeRe);
        const newBody = lines.join('\n');
        if (newBody === note.body) return;
        const _still = await joplin.workspace.selectedNote();
        if (_still?.id !== note.id) return;
